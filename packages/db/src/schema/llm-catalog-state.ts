 ...
 *
 * 「单行」由 `llm_catalog_state_singleton` 约束强制：`id` 是主键**并不**阻止
 * 第二行（id=2）落库，而 `bumpCatalogVersion` 只写 `WHERE id = 1`——多出来的行
 * 会让 M3 的轮询兜底读到一个永远没人 bump 的版本位。
 *
 * 种子行由 migration 末尾的
 * `INSERT INTO "llm_catalog_state" ("id","version") VALUES (1,0) ON CONFLICT DO NOTHING`
 * 写入——drizzle 不会替我们生成它。
 */
export const llmCatalogState = pgTable(
  'llm_catalog_state',
  {
  ...
  (t) => [check('llm_catalog_state_singleton', sql`${t.id} = 1`)]
);
